...
eval (OctoList (f:xs)) env = case eval f env of
    (OctoClosure vars f_env body) -> apply (OctoClosure vars env body) args
...
repl env = do
  putStr ">> "
  command <- getLine
  let out = (octoshow (eval (parse command) env))
  putStrLn (out)
  repl env
...
tests = TestList [
  ...
  testeval nested_let3 (OctoInt 113), -- This test fails with dynamic scoping because a different binding is used
  testeval let_test_closure (OctoInt 110), -- This test fails with dynamic scoping because the reference to n is lost
  ...
  testeval letrec_curried_map (evparse "'(20 40 60)"), -- This test fails with dynamic scoping because the reference to f is lost
...
--    (OctoString "fish clam squid"),
  ...
  testeval "(let ((f (lambda (x) (+ x y))))(let ((y 10)) (f 20)))" (OctoInt 30)
  ]
